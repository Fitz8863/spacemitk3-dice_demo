...
class Config:
    ...
    @property
    def core_arch(self) -> str:
        """Get/set EP inference core arch (K3): 'x100' (general cores 0-7, default) or 'a100' (compute cores 8-15)"""
        return self._config.extra_params.get("core_arch", "x100")

    @core_arch.setter
    def core_arch(self, value: str):
        params = self._config.extra_params
        if value in ("x100", "a100"):
            params["core_arch"] = value
        else:
            params.pop("core_arch", None)  # auto/其他: 不干预, EP 默认 (A100)
        self._config.extra_params = params
