...
//   --jsonl      stdout 改为机器可读 JSON Lines：
//                {"type":"partial","text":...}   增量识别（每 320ms）
//                {"type":"sentence","text":...}  VAD 断句整句
//                {"type":"final","text":...}     流结束的尾部文本
//                {"type":"stats",...}            结束统计
//                模型加载日志走 stderr，stdout 只有 JSON 行。
//
// 断句（VAD）:
...
// --jsonl：stdout 只输出 JSON Lines 事件，供下游程序解析
bool g_jsonl = false;
...
// JSON 字符串转义。识别文本是 BPE 拼接（正常无控制字符/引号），防御性处理。
...
// 输出一行 {"type":"...","text":"..."}；std::endl 保证管道下游即时收到
void EmitTextEvent(const char* type, const std::string& text) {
    std::cout << "{\"type\":\"" << type << "\",\"text\":\"" << JsonEscape(text)
              << "\"}" << std::endl;
}
...
void PrintPartial(double audio_sec, const std::string& text, size_t* last_len) {
    if (g_jsonl) {
        if (!text.empty()) EmitTextEvent("partial", text);  // 全量增量文本，不截断
        return;
    }
    std::string line =
...
void PrintStats(const zstream::StreamingASR& asr) {
    ...
    double rtf = audio > 0 ? infer / audio : 0.0;
    if (g_jsonl) {
        char buf[192];
        snprintf(buf, sizeof(buf),
                 "{\"type\":\"stats\",\"audio_seconds\":%.2f,\"infer_seconds\":%.2f,"
                 "\"rtf\":%.3f,\"chunks\":%d,\"tokens\":%d}",
                 audio, infer, rtf, asr.chunk_count(), asr.emitted_tokens());
...
void PrintUsage() {
    std::cout
            ...
            << "  --jsonl            stdout 输出 JSON Lines 事件（partial/sentence/final/stats）\n"
...
int RunWav(zstream::StreamingASR& asr, const std::string& wav, bool realtime,
    ...
    if (!final_text.empty()) {
        if (g_jsonl) {
            // 尾部文本无条件输出（jsonl 模式下 last_len 恒为 0，不能用它判断）
            EmitTextEvent("final", final_text);
        } else if (vad.enabled && last_len > 0) {
            PrintSentence(asr.audio_seconds(), final_text);
            sentences.push_back(final_text);
        } else if (!vad.enabled) {
            std::cout << "识别结果: " << final_text << "\n";
        }
    } else if (last_len > 0 && !g_jsonl) {
        std::cout << "\n";
    }

    if (vad.enabled && !sentences.empty() && !g_jsonl) {
...
int RunPcm(zstream::StreamingASR& asr, EnergyVad& vad) {
    if (!g_jsonl) {
        std::cout << "stdin 读取 s16le/16kHz/mono 裸流，说话即出字，Ctrl-C 结束\n"
                  ...
                  << (vad.enabled
                              ? "(VAD 断句开: 停顿约 " +
                                        std::to_string(vad.pause_ms / 1000.0)
                                                .substr(0, 3) +
                                        "s 出整句)\n"
                              : "(VAD 断句关: 文本持续累积)\n");
    }
    ...
    std::string final_text = asr.InputFinished();
    if (!final_text.empty()) {
        if (last_len > 0 && !g_jsonl) std::cout << "\n";
        if (g_jsonl) {
            EmitTextEvent("final", final_text);
        } else if (vad.enabled) {
