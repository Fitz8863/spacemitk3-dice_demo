...
CtcModel::CtcModel(const Config& config)
    ...
    if (config.provider == "spacemit") {
        ...
        // K3 核架构: "x100" 让 EP 推理线程绑 X100 通用核(0-7);
        // 其余值不干预 (EP 默认绑 A100 算力核 8-15)。双通道设置原因同 SenseVoice。
        if (config.core_arch == "x100") {
            ep_options["SPACEMIT_EP_PERFER_CORE_ARCH"] = "x100";
            setenv("SPACEMIT_EP_PERFER_CORE_ARCH", "x100", 0);
        }
        Ort::Status status =
                Ort::SessionOptionsSpaceMITEnvInit(session_options_, ep_options);
        if (status.IsOK()) {
            std::cout << "[Zipformer] SpaceMIT EP initialized (threads="
                        << config.num_threads << ", core_arch="
                        << (config.core_arch.empty() ? "ep-default" : config.core_arch)
                        << ")" << std::endl;
        } else {
