...
bool parse_args(int argc, char** argv, AppConfig& config,
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (key == "--conf" || key == "--kpt-conf" || key == "--iou") {
            if (!need_value(i, argc, argv, value)) return false;
            try {
                if (key == "--conf") config.conf = std::stof(value);
                else if (key == "--kpt-conf") config.kpt_conf = std::stof(value);
    ...
    if (config.width <= 0 || config.height <= 0 || config.fps <= 0 || config.intra_threads < 1 ||
        config.conf < 0.0f || config.conf > 1.0f || config.kpt_conf < 0.0f || config.kpt_conf > 1.0f ||
        config.iou < 0.0f || config.iou > 1.0f ||
...
int main(int argc, char** argv) {
    ...
    if (!config.yolov8_enabled) {
        std::cerr << "yolov8_enabled=false is not supported by this pose executable\n";
        return 2;
    }
    ...
    if (!no_display && config.display_enabled) {
        cv::namedWindow("YOLOv8-pose Camera", cv::WINDOW_NORMAL);
        cv::resizeWindow("YOLOv8-pose Camera", config.width, config.height);
    ...
    while (!stop.load(std::memory_order_acquire) && !stop_requested) {
        ...
        if (const auto captured = display_queue.try_pop_latest()) {
            ...
            if (last_result && last_result->captured &&
                ...
                draw_detections(bgr, last_result->detections, config.kpt_conf);
            }
            ...
            if (display_initialized) {
                cv::imshow("YOLOv8-pose Camera", bgr);
                displayed_count.fetch_add(1, std::memory_order_relaxed);
