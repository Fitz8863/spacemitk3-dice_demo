...
struct CircleParams {
    ...
    // --- 环带验证（核心判据）----------------------------------------------
    // 盘的物理身份 = "白盘被一圈深色环包住"，require_ring 时两关全过：
    //   ring_ratio       外带 {1.08,1.16,1.24} "非白"角度覆盖 >= ring_min_ratio
    //                    （周围确实有东西包着——环或地垫都算）
    //   ring_dark_ratio  内带 {0.94..1.10}（步长 0.04）逐角度取最小 V 和最小 S，
    //                    命中 = minV < ring_val_max（深环）**或** [minS < ring_sat_max
    //                    且 minV < rim_v - ring_gray_margin]（浅灰环）。★ 只有绝对
    //                    暗度会在盘种上翻车：2026-09-18 实时场景换了个浅灰环的盘
    //                    （环 V~150-190、S~14-68），暗度覆盖在 0.55 门槛上颤抖 ->
    //                    时检时不检。灰通道两条腿各堵一个漏洞：S 低排除彩色地垫
    //                    （S~184-201），V 低于盘面一截排除候选自身蹭进采样带的白
    //                    像素（白：S 低 V 高；灰环：S 低 V 中 —— 自检场景 3 抓的）。
    //   ★ 故意没有 contrast 下限：盘上的骰子/阴影会拖低内盘平均亮度（真实帧实测
    //     contrast=-3.3 的好盘），contrast 仅作诊断字段输出。
    bool   require_ring      = true;
    int    ring_val_max      = 130;   // 环带"暗"判据：minV 低于此值才算深色环 (0..255)
    int    ring_sat_max      = 90;    // 环带"灰"判据 S 上限：低于它且比盘面暗一截 = 灰环
    int    ring_gray_margin  = 45;    // 灰判据的亮度差：V 需 < rim_v - 此值（排白不排灰）
    double ring_min_ratio    = 0.55;  // 环带非白/暗(灰)的角度覆盖比例下限
