...
struct CircleParams {
    ...
    //                    （周围确实有东西包着——环或地垫都算）
    //   ring_dark_ratio  内带 {0.94..1.10}（步长 0.04）逐角度取最小 V，< ring_val_max
    //                    的角度覆盖 >= ring_min_ratio —— 这是"深色环"与"亮色地垫"
    //                    的分水岭。★ 真实暗环细且贴内（径向剖面实测：f≈0.95~1.05 处
    //                    V 中位 103~127，f>=1.08 已是地垫 V~180-225），单点采样会整条
    //                    miss，必须窄带多点取最小。取最小值后真实盘覆盖 91~100%，
    //                    "白块+亮地垫"为 ~0%。
    ...
    double ring_min_ratio    = 0.55;  // 环带非白/暗的角度覆盖比例下限
