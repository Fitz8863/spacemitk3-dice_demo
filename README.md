...
**摄像头与推流在进入游戏时就起来了**，不必等到裁决：`create_round` 会以 prewarm
模式拉起视觉 runtime，摄像头采集和 RTSP 推流（`rtsp://127.0.0.1:8554/dice/det`）立即
就绪，检测器会话同时载入，但 `adjudication_active` 保持 false——**不跑 OpenCL 预处理、
也不跑推理**，直到裁决阶段发出 `START_ADJUDICATION` 才开始计稳定帧。这样规则/准备阶段
就已经能看到台面，裁决预算里也不用再付模型加载时间。

这条流**何时结束**由全局配置 `backend/config.json` 的 `vision_always_on` 决定（见
[`backend/参数说明.md`](backend/参数说明.md)）：`true`（默认）跨回合、跨游戏常驻到服务
退出；`false` 跟随游戏生命周期，回合进入终态（按返回键退出 / 刷新 / 掉线看门狗取消）
即停流。「再来一局」在同一回合内，不受影响。该值每回合开始时读取，保存即从下一局生效。

结果按 profile 的规则生成。LLM 与 YOLO 一致时使用共识结果；不一致时先复问一次：
复问与 YOLO 一致则维持 YOLO，两次一致才允许覆盖（但点数相同的平局是算术事实，永不被覆盖成胜局），复问无定论回退 YOLO；LLM 超时也受控回退到 YOLO；其他失败、数量不符或超时进入错误，不会用网页随机数据兜底。
