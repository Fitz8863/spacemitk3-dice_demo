...
**④ 环带身份 = "暗或灰"，不能只要求"非白"（2026-09-18 收紧，当天再修一版）。**
原来环带判据是"非白覆盖 ≥55% + (contrast≥22 或 非白率≥75%)"——地垫其实很亮
（V 中位 ~180-225，与白垫只差几十级，contrast 轻松为正），贴地垫的白色物体
（如银色骰盅顶面）会被完整放行。径向 V 剖面还发现**真实暗环细且贴内**
（白垫边界外 f≈0.95~1.05），旧采样 {1.08,1.16,1.24} 基本落在地垫上。
最终设计：**f=0.94~1.10 窄带逐角度取最小 V / 最小 S**，角度命中 =
`minV < ring_val_max(130)` **或** `minS < ring_sat_max(90)`，覆盖 ≥55%。
"或灰"这条是当天实时翻车的教训：现场换了个**浅灰环的盘**（环 V~150-190、
S~14-68），纯暗度覆盖在 0.55 门槛上颤抖 → 时检时不检；而"去饱和"跨盘种、
跨光照都稳（地垫是彩色的 S~184-201，环是灰的）。contrast 下限刻意不加
（盘上骰子/阴影会拖低内盘均值，实测 contrast=-3.3 的好盘）。
自检场景 8（无环白盘应拒）/ 9（浅灰环盘应被 mask 路径接受）分别锁两头。
...
| | `require_ring` / `ring_*` | 环带验证（`ring_val_max`=暗判据、`ring_sat_max`=灰判据） |
...
9 项合成图测试，共 22 条断言（含"白色方块不能误判成圆"、"盘边搭骰子"、
"斜视 1.6:1 椭圆"、"盘心压 45% 深色圆盖"、"无深色环的白盘不能误认"、
"浅灰环盘必须被 mask 路径接受"六个难例），无需摄像头。
失败时会自动打印候选诊断。
### 跑真实帧
...
│   └── main.cpp                 CLI/配置合并、JSON 输出、9 项自检、推流接线
├── tools/eval.sh                一键回归（自检 + 批量 + 耗时表）
└── samples/                     3 张真实采集帧（含"骰子搭边"这种难例）
```

`--self-test` 的 9 项（共 22 条断言，不需要摄像头）：
...
| 8 | 白盘直接贴地垫上（无深色环） | **环带身份判据：白块 + 亮地垫不被误认** |
| 9 | 浅灰环的盘（V~190，不"暗"但"灰"） | **去饱和通道：换盘种不再时检时不检** |
