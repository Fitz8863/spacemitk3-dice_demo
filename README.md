...
| `roi` | 识别区域门限 + **推理裁剪**：`{enabled, x, y, w, h}`（占推流画面的比例）。启用后模型只看该区域（OpenCL 采样坐标重映射，手在模型输入中放大约 1.8×，小目标检出更稳），检测框坐标自动平移回全帧；同时只保留框中心落在区域内的检测，把机械臂所在的另一半画面确定性排除。推流画面画白色 ROI 边框供对位。默认 enabled=true、下半画面（y=0.5, h=0.5）。越界配置启动即报错。 |
