...
- 输出数量为 2 → 标准 Ultralytics 布局（`[1,116,8400]` + `[1,32,160,160]`），box 已在图内解码为 cx,cy,w,h；类别分与 SpaceMIT 导出约定一致，是图内已激活的概率，直接使用（官方原始 logit 导出不适用，详见 `models/README.md`）。
- 两种布局共用同一套 NMS、mask 组装和坐标映射；类别数分别从各自输出形状推导。
- 设置环境变量 `YOLO_SEG_DEBUG=1` 可在首次推理时打印输出各段数值统计（排查分数域/量化约定问题）。
