...
5. 如果启用 LLM 复核（`llm.enabled`），provider 将稳定帧和本局 prompt 作为一次无历史的 OpenAI-compatible 多模态请求。LLM 只负责复核图片，不接收其他请求上下文。**失败诊断与复核无关**（2026-09-14 起）：等不到稳定观测、或稳定观测的每侧数量不符时，原因由本地规则依据检测证据（每侧数量、分界线、是否检测为空）生成，不发任何多模态请求。
