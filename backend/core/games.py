...
def validate_asr_section(asr: Any, machine: dict[str, Any]) -> dict[str, Any]:
    ...
    Voice game selection is deliberately NOT configured here: the game list
    is a deployment-level surface, its vocabulary lives in the arena config
    (``game_select.phrases`` in ``backend/config.json``).
    """
